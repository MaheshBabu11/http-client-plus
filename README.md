...
A comprehensive Postman-like tool window for IntelliJ IDEA that lets you compose, organize, and execute HTTP requests.
All requests are saved as standard IntelliJ HTTP Client (.http) files, making them portable, shareable, and
version-controlled.
...
Each build creates a versioned ZIP (e.g., `http-client-plus-1.0.5-243.zip`) in both `build/distributions/` and
`dist/1.0.5/`.
